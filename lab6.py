...
class App(tk.Tk):
    def __init__(self):
        ...
        tk.Button(self.group_1, text="Рассчитать", command=self.on_calc_task20).grid(pady=10, sticky=tk.E)      
        ...
        tk.Radiobutton(self.group_2, text="рейтинг: 0", variable=self.check_rate, value=0, command=self.change_entry_state).grid(sticky=tk.W)
    ...
    def on_calc_task20(self):
        ...
        if not valid(): 
            ...
            self.char_mark.set(f"Буквенная оценка: {Task19.get_char_mark(num_mark)}")
